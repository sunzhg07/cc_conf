...
from hffrac import HFfracCalculation
...
default_values =    {
                            ...
                            "precalc_hf"            : False,
                            "precalc_density"       : False,

                            "constrained_hf"        : False,
                            "restricted_sort"       : True,
                            "prolate_filling"       : True,
                            "fractional_filling"    : False,

                            "q2_value"  :  55,
                            "alm_parameter"  : 0.004,
                            "nat_modelspace_in_nmax" : 14,
                            ...
                            "cc_approx"             : "ccsd",
                            "precalc_cc_gs"         : "no",
                            "precalc_cc_left_gs"    : "no",
                            "cc_calculation"        : "groundstate",
                            "expectation_value"     : "rp",
...
class EomState(object):
    ...
    def __init__(this, c, j, tz, pi, n):
        this.radius = False
        if "calculate_radius" in c : this.radius = c['calculate_radius']
        this.check_com_contamination = False
        if 'check_com_contamination' in c :
...
class Modelspace(object):
    ...
    def __init__(this, nmax, hw, nat_nmax, n3max):
        this.nmax = nmax
        this.hw = hw
        this.n3max = n3max
        this.nat_nmax = nat_nmax
    def __str__(this):
        s = "\t\tnmax = %g, hw = %g, nat_nmax = %g, n3max = %g" % (this.nmax, this.hw, this.nat_nmax, this.n3max)
        return s
    
    def get_threebody_nmax(this): return this.n3max
    def get_twobody_nmax(this): return this.nmax
    def get_nat_twobody_nmax(this): return this.nat_nmax
    def get_hw(this) : return this.hw
    @staticmethod
    def parse_outer(nmax, hws, nat_nmax, n3max=None):
        use_n3max = True
        if not n3max: use_n3max = False

        print(nmax, hws, n3max, nat_nmax)
        res = []
        for n in nmax:
            for hw in hws:
                for nnat in nat_nmax:
                    if not use_n3max: n3max = [n]
                    for n3 in n3max:
                        res.append(Modelspace(n, hw, nnat, n3))
    ...
    def parse(configtree):
        ...
        for space in configtree:
            ...
            nmax = space['nmax']
            hw = space['hw']
            nat_nmax = space['nat_nmax']
            if "n3max" in space:
                n3max = space['n3max']
                res[label] = Modelspace.parse_outer(nmax, hw, nat_nmax, n3max)
            else: res[label] = Modelspace.parse_outer(nmax, hw, nat_nmax)
            
        return res

class Core(object):
    def __init__(this, label, np,npj, nn,nnj, fractional_filling,fracfill_file,
                 np_fermi, nn_fermi, prolate_filling, mass):
        this.label = label
        this.proton_holes = np
        this.proton_holes_jscheme = npj
        this.neutron_holes = nn
        this.neutron_holes_jscheme = nnj
        this.fractional_filling = fractional_filling
        this.fracfill_file = fracfill_file
        this.proton_holes_fermi = np_fermi
        this.neutron_holes_fermi = nn_fermi
        this.prolate_filling = prolate_filling
        this.mass_correction = mass

    def get_nprotons(this) : return this.proton_holes
    def get_nprotons_jscheme(this) : return this.proton_holes_jscheme
    def get_nneutrons(this) : return this.neutron_holes
    def get_nneutrons_jscheme(this) : return this.neutron_holes_jscheme
    def get_fractional_filling(this) : return this.fractional_filling
    def get_fracfill_file(this) : return this.fracfill_file
    def get_nprotons_fermi(this) : return this.proton_holes_fermi
    def get_nneutrons_fermi(this) : return this.neutron_holes_fermi
    def get_prolate_filling(this) : return this.prolate_filling
    ...
    def __eq__(this, other):
        ...
        if this.proton_holes_jscheme!= other.proton_holes_jscheme: res = False
        if this.neutron_holes != other.neutron_holes: res = False
        if this.neutron_holes_jscheme != other.neutron_holes_jscheme: res = False
        if this.fractional_filling != other.fractional_filling: res = False
        if this.fracfill_file != other.fracfill_file: res = False
        if this.proton_holes_fermi != other.proton_holes_fermi: res = False
        if this.neutron_holes_fermi != other.neutron_holes_fermi: res = False
        if this.prolate_filling != other.prolate_filling: res = False
    ...
    def __str__(this):
        s = "%s, %s, %s, %s" % \
            (this.label, this.proton_holes, this.neutron_holes,this.proton_holes_fermi,
             this.fractional_filling, this.prolate_filling, this.neutron_holes_fermi, this.mass_correction)
        return s
...
class Run(object):
    ...
    def __init__(this, config, core, state):
        ...
        for key in this.keys:
            if key in config: this.config[key] = config[key]

        for key in this.optional_keys:
            val = False
            if key in config : val = config[key]
            this.config[key] = val
    ...
    def parse(configtree):
        ...
        for r in configtree:
            for mass in r['mass_correction']:
                core = Core(    r['core'],
                                r['proton_holes'],
                                r['proton_holes_jscheme'],
                                r['neutron_holes'],
                                r['neutron_holes_jscheme'],
                                r['fractional_filling'],
                                r['fracfill_file'],
                                r['proton_holes_fermi'],
                                r['neutron_holes_fermi'],
                                r['prolate_filling'],
                                mass )
                if ('states' in r):
...
class Calculation(object):
    ...
    def generate_hf_frac_calculations(this, nolabels):
        res = []

        for core in this.cores:
            radius = this.get_radius_flag(core)
            com = this.get_com_flag(core)
            res.extend(HFfracCalculation.factory(this.config, this.interactions,\
...
class Configuration(object):
    ...
    def generate_hf_frac_calculations(this, nolabels):
        ...
        for c in this.calculations:
            calcs.extend(c.generate_hf_frac_calculations(nolabels))
        for c in calcs:
    ...
    def write_config_files(this, filename, nolabels=False):
        hffrac=this.generate_hf_frac_calculations(nolabels)
        ...
        fh_main.write("num_hf_frac_runs = %d\n" % len(hffrac))
        ...
        for i, c in zip(range(1, len(hffrac)+1), hffrac):
            filename = "hf_frac_conf_%04d.conf" % i
            fh_main.write("hf_config_filename_%04d = %s\n" % (i, filename))
            fh_conf = open(filename, 'w')
            for k,v in c.items():
                if v == None:
...
class ConfigFactory(object):
    ...
    def factory(filename):
        ...
        # Parse parameters
        if "parameters" in content: parameters = Parameters(content["parameters"])
