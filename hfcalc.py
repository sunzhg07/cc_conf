...
class HFCalculation(OrderedDict):
...
#            this['hf_output_file_twobody_rest'] = this.get_int_outfile()
    ...
    def factory(config, interactions, modelspaces, core, radius = False, com = False, nolabels=False):
        ...
        for interaction in interactions:
            for space in modelspaces:
                calc = HFCalculation(config, interaction, space, core, nolabels)
                res.append(calc)
                if radius: res.append(HFCalculation(config, interaction, space, core, nolabels, radius=radius))
                if com: res.append(HFCalculation(config, interaction, space, core, nolabels, com=com))
    ...
    def get_cc_w0w1w2_outfile(this,config):
        return "%s_w0w1w2_ode_%s.dat" % (config["cc_approx"],
            this.get_outfile_tail())
